 ...
 * =============================================================================
 * Displays a paginated list of upcoming scheduled matches.
 * 
 * Features:
 * - Pagination with Previous/Next buttons
 * - Optional game filter
 * - Shows match details: time, format, stream, info, map veto status
 * 
...
function buildUpcomingEmbed(matches, page, gameFilter = null) {
    ...
    const embed = new EmbedBuilder()
        .setTitle(`📅 Upcoming Matches${filterText}`)
        .setColor(0x5865F2)
        .setTimestamp();
    ...
    pageMatches.forEach((match, index) => {
        ...
        // Map veto status indicator
        const mapVetoStatus = gameConfig?.hasMapVeto 
            ? (match.mapVeto && match.mapVeto.length > 0 ? '✅' : '❌') 
            : '➖';
        ...
        description += `> 🎯 Format: ${match.bestOfDisplay || 'N/A'}\n`;
        description += `> 📺 [Stream](${match.streamLink})`;
        
        if (match.infoLink && match.infoLink !== 'N/A') {
            description += ` • [Info](${match.infoLink})`;
        }
        description += '\n';
        
        if (gameConfig?.hasMapVeto) {
            description += `> 🗺️ Map Veto: ${mapVetoStatus}\n`;
        }
...
function buildPaginationButtons(page, totalPages, gameFilter = null) {
    ...
    return new ActionRowBuilder().addComponents(
        new ButtonBuilder()
            .setCustomId(`upcoming_prev_${page}${filterSuffix}`)
            .setLabel('◀ Previous')
            .setStyle(ButtonStyle.Secondary)
            .setDisabled(page === 0),
        new ButtonBuilder()
            .setCustomId(`upcoming_next_${page}${filterSuffix}`)
            .setLabel('Next ▶')
            .setStyle(ButtonStyle.Secondary)
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('upcoming')
        .setDescription('View all upcoming scheduled matches')
    ...
    async execute(interaction) {
        ...
        if (matches.length === 0) {
            ...
            return interaction.editReply({
                content: `📭 No upcoming matches scheduled${filterText}.\n\nUse \`/add\` to schedule a new match!`,
            });
        }

        // Build response
        const totalPages = Math.ceil(matches.length / MATCHES_PER_PAGE);
        const embed = buildUpcomingEmbed(matches, 0, gameFilter);
        
        const response = { embeds: [embed] };
        
        // Only add buttons if there's more than one page
        if (totalPages > 1) {
            response.components = [buildPaginationButtons(0, totalPages, gameFilter)];
        }
